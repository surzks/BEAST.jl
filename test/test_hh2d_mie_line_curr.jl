...
rc = 10.0*a # points on which field is to be calculated
...
circle = CompScienceMeshes.meshcircle(a,h/2)
...
Ht_sca_num = -potential(HH2DDoubleLayerTransposedNear(𝒟ᵀ), pts, j_TMEFIE_lc, X0; type=SVector{2, ComplexF64})
norm(Ref(ẑ) .× Ht_sca_num - Ht_sca_ana) ./ norm(Ht_sca_ana)

# usage of the TMMFIE current results in a loss of accuracy
Ht_inc = -1.0 / (im * ω * μ0) * curl(Ez_lc_Einc)
...
# Let's first compare the incident fields
#=
 Let's place the source at ρ = 2.0 and φ = 0.0, and a circle of radius r_inc = 1.0
 with it's centre at the origin. We compute the incident magnetic fields on the 
 circumference of the circle, both analytically and numerically and compare the two,
 which, naturally, are supposed to match.
=#
I = 1.0
ρp = 2.0
φp = 0.0
...
function Hz_sca(I::F,k::F,a::F,ρ::F,φ::F,ρp::F,φp::F; Z = ComplexF64(0)) where F
    
    val(n) = dhankelh2(n,k*ρp)/An_TE(a,n) * hankelh2(n,k*ρ) * exp(im*n*(φ-φp))
    ret = val(0)
...
function Hz_sca(I::F, k::F, a::F, pts::Vector{SVector{2,F}}, spt::SVector{2,F}) where F
    return [Hz_sca(I, k, a, cart2polar(p[1],p[2])..., spt[1], spt[2]) for p in pts]
end

Hz_sca_ana = Hz_sca(I, k, a, pts, SVector(ρp, φp))
...
norm( Hz_sca_ana - Hz_sca_num) /norm(Hz_sca_ana)

##
# TE-EFIE
# Analytical

function Et_sca(I::F,k::F,a::F,ρ::F,φ::F,ρp::F,φp::F; Z = ComplexF64(0)) where F

    valEρ(n) = n * dhankelh2(n,k*ρp)/An_TE(a,n) * hankelh2(n,k*ρ) * exp(im*n*(φ-φp))
    valEφ(n) = dhankelh2(n,k*ρp)/An_TE(a,n) * dhankelh2(n,k*ρ) * exp(im*n*(φ-φp))
    ...
    # Prefactor 1
    #retEρ *= im/ρ * 1.0/(ε0*μ0) # k^2/ε0
    #retEφ *= -k * 1.0/(ε0*μ0) # k^2/ε0
    ...
    retEρ *= I*k^2/(4*im) * im/ρ * (1/im*ω*ε0)
    retEφ *= I*k^2/(4*im) * k * (1/im*ω*ε0)
 
    
    return retEρ * SVector(cos(φ),sin(φ)) + retEφ * SVector(-sin(φ),cos(φ))
end

function Et_sca(I::F, k::F, a::F, pts::Vector{SVector{2,F}}, spt::SVector{2,F}) where F
    return [Et_sca(I, k, a, cart2polar(p[1],p[2])..., spt[1], spt[2]) for p in pts]
end

Et_sca_ana = Et_sca(I, k, a, pts, SVector(ρp, φp))

# Numerical
Et_inc = Helmholtz2D.directedmonopole(;position=SVector(xp,yp),
    direction=SVector(0.0,1.0),
    wavenumber=k,
    amplitude=1.0)
et = assemble(TangentTrace(Et_inc),X1)
##
...
norm( Et_sca_num_curl - Et_sca_ana) ./ norm(Et_sca_ana)
